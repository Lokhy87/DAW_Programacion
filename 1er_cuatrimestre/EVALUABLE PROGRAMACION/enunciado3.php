<?php // Cuantas casillas en que fila tienen n numero 
    session_start(); 
...
<?php
    ...
    // Cuántas casillas en una fila específica tienen el número 4
    function cuentacasillas($tablero, $fila) {
        $numero = 4;
        $n_total = 0;

        // Verificar si la fila está dentro de los límites del tablero
        if ($fila < 1 || $fila > 6) {
            echo "Número de fila no válido. Debe ser entre 1 y 6.";
            return;
        }

        // Recorrer la fila especificada (ajustando el índice a 0 basado)
        foreach ($tablero[$fila - 1] as $celda) {
            // Separar número y color
            list($numero_celda, $color_celda) = explode("-", $celda);

            // Verificar si el número es 4
            if ($numero_celda == $numero) {
                $n_total++;
            }
        }

        echo "El número $numero aparece $n_total veces en la fila $fila.";
    }

    // Llamada a la función, por ejemplo, para la fila 3
    cuentacasillas($tablero, 1);
